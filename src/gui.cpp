...
bool NoriScreen::dropEvent(const std::vector<std::string> &filenames) {
    if(filenames.size() > 0) {
...
bool NoriScreen::keyboardEvent(int key, int scancode, int action, int modifiers) {
    if(action && key == GLFW_KEY_O && modifiers & GLFW_MOD_CONTROL) {
        using FileType = std::pair<std::string,std::string>;
        std::vector<FileType> filetypes = { FileType("xml", "Nori Scene File"), FileType("exr", "EXR Image File") };
        std::string filename = nanogui::file_dialog(filetypes, false);
        dropEvent({ filename });
    ...
    if(action && key == GLFW_KEY_Z && modifiers & GLFW_MOD_CONTROL) {
        m_renderThread.stopRendering();
        return true;
    }
